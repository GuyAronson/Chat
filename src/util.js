...
import { isUserExists, createUser, getUser } from './fetch';
...
export async function checkSubmitValidation({event, setUser}){
    ...
    clearErrors();
    let u = await isUserExists(username);
    ...
    //     document.querySelector('#passErrorMesage').innerHTML = "Password is invalid. Need to be greater than 7 characters";
    //     formIsValid = false
    // }
    ...
    if(formIsValid) {
        ...
        let user = await createUser(username, nickname, password, email);
